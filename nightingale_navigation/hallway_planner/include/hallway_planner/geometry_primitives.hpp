...
// path in pixel space
using PixelPath = std::vector<Point>;
...
public:
    ...
    /* Get rho, the shortest distance from the line to the origin. this is from the polar repr */
    double rho() const;
    /* Get theta, the angle of the line's normal vector. this is from the polar repr */
    double theta() const;
...
{
    std::size_t operator()(const hallway_planner::geometry_primitives::Point& p) const noexcept
    {
